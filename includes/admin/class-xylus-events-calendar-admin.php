...
class Xylus_Events_Calendar_Admin
{
	...
	public function __construct()
	{
		...
		add_shortcode('easy_event_calendar_mini', array($this, 'xylusec_calendar_mini_shortcode'));
	...
	public function xylusec_calendar_shortcode($atts)
	{
		...
		$is_mini = (isset($atts['layout']) && $atts['layout'] === 'mini_calendar') ? true : false;

		$inline = 'if (window.xylusec_ajax) { ';
		if ($is_mini) {
			$inline .= 'xylusec_ajax.is_mini = true; ';
		}
		$inline .= 'xylusec_ajax.shortcode_atts = ' . wp_json_encode($atts) . '; }';
		wp_add_inline_script('xylus-events-calendar', $inline, 'before');

		ob_start();
		if ($is_mini) {
			include XYLUSEC_PLUGIN_DIR . 'templates/admin/xylus-events-calendar-mini-template.php';
		} else {
	...
	public function xylusec_calendar_mini_shortcode($atts)
	{
		wp_enqueue_script('xylus-events-calendar');
		$inline = 'if (window.xylusec_ajax) { xylusec_ajax.is_mini = true; xylusec_ajax.shortcode_atts = ' . wp_json_encode($atts) . '; }';
		wp_add_inline_script('xylus-events-calendar', $inline, 'before');
					...
					<div class="eec-related-card">
						<div class="eec-related-image-wrap">
							<a href="<?php the_permalink(); ?>">
								<img src="<?php echo esc_url($image ?: $fallback_image); ?>"
	...
	public function eec_events_discovery_shortcode($atts)
	{
		...
		$all_collections = get_terms(array('taxonomy' => 'eec_collection', 'hide_empty' => true));
		...
		$filter_collection = isset($_GET['eec_collection']) ? sanitize_text_field(wp_unslash($_GET['eec_collection'])) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		...
		<div class="eec-archive-wrap eec-discovery-shortcode">
			<div class="eec-filter-bar">
				<form method="get" class="eec-filter-form" id="eec-filter-ajax-form">
					<div class="eec-filter-search">
						...
						<input type="text" name="eec_search"
							placeholder="<?php esc_attr_e('Search events...', 'xylus-events-calendar'); ?>"
							value="<?php echo esc_attr($filter_search); ?>" class="eec-filter-input" />
					</div>
					...
					<div class="eec-filter-select-wrap">
						<select name="eec_collection" class="eec-filter-select">
							...
							<?php foreach ($all_collections as $col): ?>
								<option value="<?php echo esc_attr($col->slug); ?>" <?php selected($filter_collection, $col->slug); ?>><?php echo esc_html($col->name); ?></option>
							<?php endforeach; ?>
						</select>
					</div>
					...
					<button type="submit"
						class="eec-filter-btn"><?php esc_html_e('Filter', 'xylus-events-calendar'); ?></button>

					<div class="eec-layout-toggle" id="eec-layout-toggle-group">
						<input type="hidden" name="eec_layout" id="eec-layout-input" value="grid" />
						<button type="button" class="eec-layout-btn active" data-layout="grid"
							title="<?php esc_attr_e('Grid View', 'xylus-events-calendar'); ?>">
						...
						<button type="button" class="eec-layout-btn" data-layout="list"
							title="<?php esc_attr_e('List View', 'xylus-events-calendar'); ?>">
							<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"
								width="20" height="20">
								<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
					...
					<?php if ($filter_search || $filter_category || $filter_collection || $filter_venue || $filter_organizer): ?>
						<a href="<?php echo esc_url(get_the_permalink()); ?>"
							class="eec-filter-reset"><?php esc_html_e('Reset', 'xylus-events-calendar'); ?></a>
					<?php endif; ?>
				</form>
			</div>
			...
			<div class="eec-results-info">
				...
				$collection = isset($_GET['eec_collection']) ? sanitize_text_field(wp_unslash($_GET['eec_collection'])) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
									...
									<div class="tabs-scroller">
										<div class="var-tabs var-tabs--item-horizontal var-tabs--layout-horizontal-padding">
											<div class="var-tabs__tab-wrap var-tabs--layout-horizontal">
												<a href="<?php echo esc_url(admin_url('admin.php?page=xt_events_calendar&tab=general')); ?>"
													class="var-tab <?php echo $active_tab == 'general' ? 'var-tab--active' : 'var-tab--inactive'; ?>">
													<span
														class="tab-label"><?php esc_attr_e('General', 'xylus-events-calendar'); ?></span>
												</a>
												<a href="<?php echo esc_url(admin_url('admin.php?page=xt_events_calendar&tab=widget')); ?>"
													class="var-tab <?php echo $active_tab == 'widget' ? 'var-tab--active' : 'var-tab--inactive'; ?>">
													<span
														class="tab-label"><?php esc_attr_e('Widget Appearance', 'xylus-events-calendar'); ?></span>
												</a>
												<a href="<?php echo esc_url(admin_url('admin.php?page=xt_events_calendar&tab=shortcode')); ?>"
													class="var-tab <?php echo $active_tab == 'shortcode' ? 'var-tab--active' : 'var-tab--inactive'; ?>">
													<span
														class="tab-label"><?php esc_attr_e('Shortcode', 'xylus-events-calendar'); ?></span>
												</a>
												<a href="<?php echo esc_url(admin_url('admin.php?page=xt_events_calendar&tab=support')); ?>"
													class="var-tab <?php echo $active_tab == 'support' ? 'var-tab--active' : 'var-tab--inactive'; ?>">
													<span
														class="tab-label"><?php esc_attr_e('Support & Help', 'xylus-events-calendar'); ?></span>
												</a>
											</div>
										</div>
									</div>
	...
	public function xylusec_handle_so_settings_submit()
	{
		...
		if (
			...
			$xylusec_so_options = [
				...
				'xylusec_filter_show_organizer' => isset($_POST['xylusec_filter_show_organizer']) ? 'yes' : 'no',
				'xylusec_filter_show_collection' => isset($_POST['xylusec_filter_show_collection']) ? 'yes' : 'no',
				'xylusec_filter_show_day' => isset($_POST['xylusec_filter_show_day']) ? 'yes' : 'no',
				...
				'xylusec_primary_color' => esc_attr(sanitize_text_field(wp_unslash($_POST['xylusec_primary_color'] ?? xylusec_xt_events_calendar()->common->xylusec_get_theme_primary_color()))),
	...
	public function xylusec_handle_so_widget_settings_submit()
	{
		...
		if (
			...
			// Check if this is a reset-to-defaults action
			$is_reset = isset($_POST['xylusec_reset_defaults']) && '1' === $_POST['xylusec_reset_defaults'];
			...
			if ($is_reset) {
				// Force save by deleting first, then re-adding
				delete_option(XYLUSEC_WIDGET_OPTIONS);
				update_option(XYLUSEC_WIDGET_OPTIONS, $xylusec_so_widget_options);
				$xylusec_success_msg[] = __('Widget color settings reset to defaults successfully.', 'xylus-events-calendar');
			} else {
		...
		if (get_option('xylusec_plugin_activated')) {
			...
			if (!get_option(XYLUSEC_OPTIONS)) {
				$defaults = [
					...
					'xylusec_primary_color' => xylusec_xt_events_calendar()->common->xylusec_get_theme_primary_color(),
