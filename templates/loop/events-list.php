...
/**
 * Shared template for rendering the event list loop.
 * Provides a horizontal layout for events.
 *
...
if ( $query->have_posts() ) : ?>
	...
	<div class="eec-archive-list">
		...
		<?php while ( $query->have_posts() ) : $query->the_post();
			...
			// Event status.
			$status = ( $xylusec_end_ts && $xylusec_real_time > $xylusec_end_ts ) ? 'past' : 'upcoming';
		?>

			<div class="eec-list-card <?php echo esc_attr( 'eec-status-' . $status ); ?>">
				<div class="eec-list-image-wrapper">
				...
				<div class="eec-list-content">
					<div class="eec-list-header">
						<h2 class="eec-list-title">
					...
					<div class="eec-list-excerpt">
						<?php 
						$xylusec_excerpt     = get_the_excerpt();
						if ( empty( $xylusec_excerpt ) ) {
							$xylusec_excerpt = wp_trim_words( get_the_content(), 30 );
						} else {
							$xylusec_excerpt = wp_trim_words( $xylusec_excerpt, 30 );
						}
						echo wp_kses_post( $xylusec_excerpt ); 
						?>
					</div>

					<div class="eec-list-footer">
						<a href="<?php echo esc_url( $xylusec_event_permalink ); ?>" class="eec-list-read-more">
							<?php esc_html_e( 'View Details', 'xylus-events-calendar' ); ?>
							<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" width="16" height="16">
	...
	if ( $query->max_num_pages > 1 ) : ?>
		<div class="eec-archive-pagination">
			...
			echo paginate_links( array( // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				...
				'prev_text' => '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" width="16" height="16"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" /></svg> ' . esc_html__( 'Prev', 'xylus-events-calendar' ),
				'next_text' => esc_html__( 'Next', 'xylus-events-calendar' ) . ' <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" width="16" height="16"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg>',
			) );
			?>
		</div>
	<?php endif; ?>
...
<?php else : ?>
	<div class="eec-no-events">
		<p><?php esc_html_e( 'No events found.', 'xylus-events-calendar' ); ?></p>
	</div>
<?php endif; ?>
